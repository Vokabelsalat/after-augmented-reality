  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-[#111]">
      <div className="absolute inset-0 bg-[#0B0B0B]" aria-hidden="true">
        <div className="absolute top-1/2 left-1/2 h-[42vh] w-[68vw] max-w-xs -translate-x-1/2 -translate-y-1/2 border border-white/10" />
        <div className="absolute top-1/2 left-1/2 size-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/30" />
      </div>
      <div ref={containerRef} className="absolute inset-0 [&>canvas]:!absolute [&>canvas]:!inset-0 [&>canvas]:!h-full [&>canvas]:!w-full [&>video]:!absolute [&>video]:!inset-0 [&>video]:!h-full [&>video]:!w-full [&>video]:!object-cover" />
      ...
      {scannerState === "starting" && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/65" role="status">
          <p className="animate-pulse text-[10px] tracking-[0.25em] text-white/60 uppercase">
            Starting camera
          </p>
        </div>
      )}
      ...
      {scannerState === "running" && (
        <p className="absolute inset-x-0 bottom-[9rem] z-10 text-center text-[10px] tracking-[0.22em] text-white/60 uppercase" role="status">
