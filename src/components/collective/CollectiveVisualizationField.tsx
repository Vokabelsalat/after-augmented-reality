...
function FloatingAbstractCreature({ contribution }: { contribution: ExhibitionContribution }) {
...
function CollectiveAbstractField({ contributions }: { contributions: ExhibitionContribution[] }) {
  return (
    <Canvas orthographic camera={{ position: [0, 0, 10], zoom: 82, near: 0.1, far: 30 }} dpr={[1, 1.35]} gl={{ alpha: true, antialias: true, powerPreference: "high-performance" }}>
      ...
      {contributions.slice(-32).map((contribution) => (
        <FloatingAbstractCreature key={contribution.id} contribution={contribution} />
      ))}
...
export function CollectiveVisualizationField({ contributions }: { contributions: ExhibitionContribution[] }) {
  ...
  if (visualizationDesign === "creature") {
    return <CollectiveAbstractField contributions={contributions} />;
  }
  return <CollectiveCreatureField contributions={contributions} />;
}
